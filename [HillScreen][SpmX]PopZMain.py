...
def input_parameters():
    ...
    simulation_time = 1000#1802.5 #sec
    time_interval = 2.5

    # The parameters for adjustment of behavior of SpmX
    Trial = 3
    SpmXSampleNum = 10
    SpmX_amount = np.array([50, 50])
    ...
    SizeThresSample = 10
    CooperSample = 1
    ParaSizeThres = np.array([10.5, 10.5])  # m
    SizeThresmode = 'linear scale'
    ParaCooper = np.array([9, 9])  # n
    ...
    Prodmode = 'log scale'
    ParaDeg = np.array([-2.5, -2.5])  # 7.5*10**(-5) + 2.8*10**(-4)  3.55*10**(-4) # 3.55*10**(-4)/3 np.array([-2.5, -1.5])
    Degmode = 'log scale'
    ParaBind = np.array([0.1, 0.1])  # 0.5
    Bindmode = 'line scale'
...
def export_results(output_dict):
    ...
    for data, paras in zip(res, para_save):
        ...
        ind +=1
    with open('result.pickle', 'wb') as picklefile:
